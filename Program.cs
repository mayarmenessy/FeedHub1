...
var feedHubRepository = new feedHubRepository(connectionString ?? string.Empty);
await feedHubRepository.CreateDatabaseTable();
builder.Services.AddSingleton(feedHubRepository);
...
app.MapPost("/login",
    async ([FromForm] string email, [FromForm] string password, feedHubRepository repo, HttpContext context) =>
    {
...
app.MapGet("/feeds", async (feedHubRepository repo, ClaimsPrincipal claimsPrincipal) =>
{
...
app.MapPost("/feeds", async ([FromForm] string feedUrl, feedHubRepository repo, ClaimsPrincipal claimsPrincipal) =>
{
    try
    {
...
app.MapGet("/feeds/{id:long}", async (long id, feedHubRepository repo, ClaimsPrincipal claimsPrincipal) =>
{
    Feed feed;
    string html;
...
app.MapDelete("/feeds/{id:int}", async (int id, feedHubRepository repo, ClaimsPrincipal claimsPrincipal) =>
{
    try
    {
...
internal class feedHubRepository(string connectionString)
{
